...
class MessageReaction:
	def __new__(self, messageObject, auto_format=True):
		self.msg_id = messageObject.msgId
		self.cli_msg_id = messageObject.cliMsgId
		self.msg_type = _util.getClientMessageType(messageObject.msgType)
		if not isinstance(msgType, int):
			raise ValueError("Msg Type must be int")
		
		if auto_format:
			self.reaction_format = [{
				"gMsgID": int(self.msg_id),
				"cMsgID": int(self.cli_msg_id),
...
class Mention:
	def __new__(self, uid, length=1, offset=0, auto_format=True):
		...
		if auto_format:
			self.mention_format = json.dumps([{
				"pos": self.offset,
				"len": self.length,
				"uid": self.user_id,
				"type": self.type
...
class MultiMention:
	def __init__(self, listMention):
		...
		self.mention_format = json.dumps(mentions)
