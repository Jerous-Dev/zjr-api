...
class ZaloAPI(object):
	...
	def login(self, phone, password, imei, user_agent=None):
		...
		try:
			...
			self.uid = self.fetch_account_info().profile.get("userId", self._state.user_id)
		except:
			self._imei = None
			self.uid = self._state.user_id
		
		self.on_logged_in(self._state._config.get("phone_number"))
	...
	def _upload_image(self, filePath, thread_id, thread_type):
	...
	def fetch_phone_number(self, phoneNumber, language="vi"):
	...
	def fetch_group_info(self, groupId):
	...
	def get_recent_group(self, groupId):
	...
	def _get_group_board_list(self, board_type, page, count, last_id, last_type, groupId):
		params = {
			"params": self._encode({
	...
	def get_group_board_list(self, groupId, page=1, count=20, last_id=0, last_type=0):
		...
		Raises:
			ZaloAPIException: If request failed
		"""
		response = self._get_group_board_list(0, page, count, last_id, last_type, groupId)
		
		return response
	
	def get_group_pin_msg(self, groupId, page=1, count=20, last_id=0, last_type=0):
		...
		Raises:
			ZaloAPIException: If request failed
		"""
		response = self._get_group_board_list(2, page, count, last_id, last_type, groupId)
		
		return response
	
	def get_group_note(self, groupId, page=1, count=20, last_id=0, last_type=0):
		...
		Raises:
			ZaloAPIException: If request failed
		"""
		response = self._get_group_board_list(1, page, count, last_id, last_type, groupId)
		
		return response
	
	def get_group_poll(self, groupId, page=1, count=20, last_id=0, last_type=0):
		...
		Raises:
			ZaloAPIException: If request failed
		"""
		response = self._get_group_board_list(3, page, count, last_id, last_type, groupId)
	...
	def change_account_setting(self, name, dob, gender, biz={}, language="vi"):
	...
	def change_account_avatar(self, filePath, width=500, height=500, language="vn", size=None):
		...
		Returns:
			...
			None: If request success/failed depending on the case
	...
	def send_friend_request(self, userId, msg, language="vi"):
		...
		Returns:
			object: `User` Friend request response
			dict: A dictionary containing error responses
	...
	def accept_friend_request(self, userId, language="vi"):
		...
		Returns:
			object: `User` Friend accept request response
			dict: A dictionary containing error responses
	...
	def block_view_feed(self, userId, isBlockFeed):
	...
	def create_group(self, name=None, description=None, members=[], nameChanged=1, createLink=1):
		"""Create a new group.
	...
	def change_group_avatar(self, filePath, groupId):
	...
	def change_group_name(self, groupName, groupId):
	...
	def change_group_desc(self, groupDesc, groupId):
		"""Not Available Yet"""
	
	def change_group_setting(self, groupId, defaultMode="default", **kwargs):
		...
		if defaultMode == "anti-raid":
			...
			defSetting = self.fetch_group_info(groupId).gridInfoMap
	...
	def change_group_owner(self, newAdminId, groupId):
	...
	def add_users_to_group(self, user_ids, groupId):
	...
	def kick_users_in_group(self, members, groupId):
	...
	def block_users_in_group(self, members, groupId):
	...
	def unblock_users_in_group(self, members, groupId):
	...
	def add_group_admins(self, members, groupId):
	...
	def remove_group_admins(self, members, groupId):
	...
	def pin_group_msg(self, pinMsg, groupId):
	...
	def unpin_group_msg(self, pinId, pinTime, groupId):
	...
	def delete_group_msg(self, msgId, ownerId, clientMsgId, groupId):
	...
	def view_group_pending(self, groupId):
	...
	def handle_group_pending(self, members, groupId, isApprove=True):
	...
	def view_poll_detail(self, pollId):
	...
	def disperse_group(self, groupId):
	...
	def send(self, message, thread_id, thread_type=ThreadType.USER, mark_message=None, ttl=0):
		...
		if message.mention:
			return self.send_mention_message(message, thread_id, ttl)
		else:
			return self.send_message(message, thread_id, thread_type, mark_message, ttl)
	
	def send_message(self, message, thread_id, thread_type, mark_message=None, ttl=0):
	...
	def reply_message(self, message, replyMsg, thread_id, thread_type, ttl=0):
	...
	def send_mention_message(self, message, groupId, ttl=0):
	...
	def send_to_do(self, message_object, content, user_ids, thread_id, thread_type, due_date=-1, desc=""):
		"""Send todo to a user or group.
	   
		Args:
			message_object (Message): The orgirinal message object
			content (str): Content todo
			user_ids (list): List user_ids to send to
			thread_id (str): User/Group ID to send to
			thread_type (ThreadType): ThreadType.USER, ThreadType.GROUP
			due_date (int): Due date (-1 if not)
			desc (str): Description (None if not)
		...
		if not assignees:
			raise ZaloAPIException("Missing recipient id")
	...
	def send_call(self, user_id, callId=None):
		"""Send call to a user.
		
		Args:
			user_id (int | str): User ID to send to
			callId (int | str): Call ID (None if not)
		...
		response = self._post("https://voicecall-wpa.chat.zalo.me/api/voicecall/requestcall", params=params, data=request_payload)
		try:
		...
		if request_data.get("error_code") != 0:
			...
			error_message = request_data.get("error_message", "Unknown errol")
			raise ZaloAPIException(f"Errol #{error_code} when sending requests: {error_message}")
		...
		if results:
			...
			error_code = request_data.get("error_code")
			error_message = request_data.get("error_message") or request_data.get("data")
			raise ZaloAPIException(f"Errol #{error_code} when sending requests: {error_message}")
		...
		payload = {
			"params": self._encode({
				...
				"codec": results.get('codec', '[{"dynamicFptime":0,"frmPtime":20,"name":"opus/16000/1","payload":112}]'),
		...
		if data.get("error_code") != 0:
			error_code = data.get("error_code")
			error_message = data.get("error_message", "Unknown errol")
			raise ZaloAPIException(f"Errol #{error_code} when sending requests: {error_message}")
	...
	def join_group(self, url):
		...
		Args:
			url (str): Link to join
			
		Returns:
			object: Resuls of joining the group
			...
			if response_data.get('error_code') == 0:
				...
				if data:
					try:
						...
						raise ValueError(f"Decode errol: {str(e)}")
				else:
					raise ValueError("Data is None")
			else:
				error_message = response_data.get('error_message', 'Unknown errol')
				error_code = response_data.get('error_code', 0)
				raise Exception(f"Errol #{error_code} when sending requests: {error_message}")
		else:
			raise TypeError(f"Unexpected response type: expected requests.models.Response, got {type(response)}")

	def leave_group(self, group_id, imei, silent=1, language="vi"):
		"""Leave current group
	   
		Args:
			group_id (int | str): ID Group 
			imei (str): IMEI account
			silent (int): Leave group mode [(1) silent; (2) announcement]
			language (str): Language (Default: Vietnamese)
			
		Returns:
			object: Results of leaving the group
	...
	def undo_message(self, msgId, cliMsgId, thread_id, thread_type):
	...
	def send_reaction(self, messageObject, reactionIcon, thread_id, thread_type, reactionType=75):
	...
	def send_multi_reaction(self, reactionObj, reactionIcon, thread_id, thread_type, reactionType=75):
		"""Reaction message by ID.
	...
	def send_remote_file(self, fileUrl, thread_id, thread_type, fileName="default", fileSize=None, extension="vrxx", ttl=0):
	...
	def send_remote_video(self, videoUrl, thumbnailUrl, duration, thread_id, thread_type, width=1280, height=720, message=None, ttl=0):
	...
	def send_remote_voice(self, voiceUrl, thread_id, thread_type, fileSize=None, ttl=0):
		"""Send voice by url.
	...
	def send_local_image(self, imagePath, thread_id, thread_type, width=2560, height=2560, message=None, custom_payload=None, ttl=0):
		...
		if custom_payload:
			...
			uploadImage = self._upload_image(imagePath, thread_id, thread_type)
	...
	def send_multi_local_image(self, imagePathList, thread_id, thread_type, width=2560, height=2560, message=None, ttl=0):
		...
		for i, imagePath in enumerate(imagePathList):
			...
			data = self.send_local_image(imagePath, thread_id, thread_type, width, height, message, custom_payload=payload)
	...
	def send_local_gif(self, gifPath, thumbnailUrl, thread_id, thread_type, gifName="vrxx.gif", width=500, height=500, ttl=0):
	...
	def send_sticker(self, stickerType, stickerId, cateId, thread_id, thread_type, ttl=0):
	...
	def send_link(self, linkUrl, title, thread_id, thread_type, thumbnailUrl=None, domainUrl=None, desc=None, message=None, ttl=0):
	...
	def send_report(self, user_id, reason=0, content=None):
		"""Send report to Zalo.
	...
	def send_business_card(self, userId, qrCodeUrl, thread_id, thread_type, phone=None, ttl=0):
	...
	def set_typing(self, thread_id, thread_type):
	...
	def mark_as_delivered(
		...
		results = data.get("data") if data.get("error_code") == 0 else None
		if results:
			self.on_message_delivered(msgId, thread_id, thread_type, _util.now())
			return True
	...
	def mark_as_read(
		...
		results = data.get("data") if data.get("error_code") == 0 else None
		if results:
			self.on_marked_seen(msgId, thread_id, thread_type, _util.now())
			return True
				...
				for message in messages + groupmsg:
					if int(message["ts"]) >= ListenTime and message["msgId"] not in HasRead:
						...
						if message in messages:
							...
							[
								pool.submit(self.on_message, msgObj.msgId, str(int(msgObj.uidFrom) or self.uid), msgObj.content, msgObj, str(int(msgObj.uidFrom) or msgObj.idTo), ThreadType.USER)
								if thread else
								self.on_message(msgObj.msgId, str(int(msgObj.uidFrom) or self.uid), msgObj.content, msgObj, str(int(msgObj.uidFrom) or msgObj.idTo), ThreadType.USER)
							]
						
						else:
							
							[
								pool.submit(self.on_message, msgObj.msgId, str(int(msgObj.uidFrom) or self.uid), msgObj.content, msgObj, str(int(msgObj.idTo) or self.uid), ThreadType.GROUP)
								if thread else
								self.on_message(msgObj.msgId, str(int(msgObj.uidFrom) or self.uid), msgObj.content, msgObj, str(int(msgObj.idTo) or self.uid), ThreadType.GROUP)
							]
		...
		with connect(url, additional_headers=headers) as ws:
			...
			while not self._condition.is_set():
				try:
					...
					n, cmd, s = _util.get_header(encodedHeader)
					...
					if n == 1 and cmd == 3000 and s == 0:
						...
						try:
							for message in groupMsgs:
								messages = self.get_recent_group(message["idTo"])["groupMsgs"]
						...
						for control in controls:
							if control["content"]["act_type"] == "group":
								...
								groupEventType = _util.get_group_event_type(control["content"]["act"])
								event_data = EventObject.fromDict(groupEventData)
								event_type = groupEventType
								[
									pool.submit(self.on_event, event_data, event_type)
									if thread else
									self.on_event(event_data, event_type)
								]
	...
	def start_listening(self, delay=1, thread=False, type="websocket", reconnect=5):
	...
	def listen(self, delay=1, thread=False, type="websocket", run_forever=False, reconnect=5):
		...
		self.start_listening(delay, thread, type, reconnect)
	...
	def on_logging_in(self, phone=None):
	...
	def on_logged_in(self, phone=None):
	...
	def on_event(self, event_data, event_type):
	...
	def on_error_call_back(self, error, ts=int(time.time())):
